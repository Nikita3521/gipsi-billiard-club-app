...
export default function CarouselFoods({ img }: CarouselProps) {
  ...
  return (
    <>
      {/* Tablet strip — narrow column, hidden below sm and above lg */}
      <div className="hidden sm:flex sm:w-28 sm:flex-col sm:gap-3 lg:hidden">
        {visibleImg.map((item) => (
          <motion.div
            ...
            className="relative aspect-square overflow-hidden border border-white/10 bg-[#0b1d3a] shadow-[0_18px_50px_rgba(0,0,0,0.3)]"
          >
            <Image
              src={item.src}
              alt={item.alt}
              ...
              sizes="112px"
            />
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(7,20,43,0.05)_0%,rgba(7,20,43,0.12)_25%,rgba(7,20,43,0.65)_100%)]" />
          </motion.div>
        ))}
      </div>
